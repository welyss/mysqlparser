	...
	public int lexOneToken(SQLThread thd) {
		...
		for (;;) {
			...
			case MY_LEX_IDENT_OR_NCHAR:
				...
				/* Found N'string' */
				lip.yySkip(thd); // Skip '
				if (!"".equals((token.lexStr.str = getText(thd, 2, 1)))) {
			...
			case MY_LEX_IDENT:
				...
				if (start == lip.getPtr(thd) && c == '.' && ident(lip.yyPeek(thd)))
					...
					if ((tokval = findKeyword(thd, length, c == '(')) > 0) {
						...
						((Token)thd.yylval).lexStr.str = thd.sql.substring(thd.mTokStart, thd.mTokStart + length);
					}
					lip.yySkip(thd); // next state does a unget
				}
				token.lexStr.str = getToken(thd, 0, length);
...
//				 }
			...
			case MY_LEX_IDENT_SEP: // Found ident and now '.'
				token.lexStr.str = String.valueOf((char) lip.getPtr(thd));
			...
			case MY_LEX_USER_VARIABLE_DELIMITER: // Found quote char
			{
				...
				if (double_quotes > 0)
					token.lexStr.str = getQuotedToken(thd, 1, lip.yyLength(thd) - double_quotes - 1, quote_char);
				else
			...
			case MY_LEX_INT_OR_REAL: // Complete int or incomplete real
				if (c != '.') { // Found complete integer number.
					token.lexStr.str = getToken(thd, 0, lip.yyLength(thd));
					return intToken(token.lexStr.str, token.lexStr.str.length());
				}
				// fall through
			...
			case MY_LEX_HEX_NUMBER: // Found x'hexstring'
				...
				token.lexStr.str = getToken(thd, 2, // skip x'
			...
			case MY_LEX_BIN_NUMBER: // Found b'bin-string'
				...
				token.lexStr.str = getToken(thd, 2, // skip b'
			...
			case MY_LEX_STRING: // Incomplete text string
				if ((token.lexStr.str = getText(thd, 1, 1)) == null) {
					state = MyLexStates.MY_LEX_CHAR; // Read char by char
					break;
				}
...
//						 lip.set_echo(TRUE);
			...
			case MY_LEX_USER_END: // end '@' of user@hostname
				...
				token.lexStr.str = String.valueOf(lip.yyPeekn(thd, -1));
				return ((int) '@');
			...
			case MY_LEX_SYSTEM_VAR:
				token.lexStr.str = String.valueOf(lip.yyGet(thd));
				lip.yySkip(thd); // Skip '@'
